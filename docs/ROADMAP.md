...
4. **Phase 3, operator spike and deterministic reduction**: test three trusted state-transition operators on one asynchronous defect and reduce a preserved violation to a deterministic 1-minimal spec.
   ...
   - Adopt when at least one operator, within the pre-registered generation budget, produces a preserved violation identity on the defect artifact that the identity-transformation baseline does not produce (or needs a strictly larger budget to produce), and that result passes the per-patch F-to-P check and five-of-five replay. Otherwise retire. Budget numbers and measurement formulas are pre-registered in `benchmark/v0.3/` before the first measured run.
...
Phase 0 is complete as of 2026-09-01 at `2ee324559323360b569105e4e5c75edd286ba46e`. Its history validators pass with the recorded unavailable image gaps. Phase 1 is complete as of 2026-09-01 at `28ad2bbe4bcfb3e8049ca2564e38b92c8af42ec9`. The registered frozen consumer lockfile resolves every direct and transitive first-party dependency from its packed tarball, the clean-consumer realpaths confirm those tarball resolutions, audit evidence pointers are bound to their registered modules and expected observations, and the isolation receipt is bound to the preparation runner, Docker arguments, and in-container observations. The `contracts` and `history` validators return 0. Phase 2 is complete as of 2026-09-01 at `28ad2bbe4bcfb3e8049ca2564e38b92c8af42ec9`. Its canonical seed-to-plan path passes four positive and nine negative cases, and its isolated trusted evaluator passes ten result-channel cases, including marker forgery, an observation kind flip, missing or malformed results, a wrong digest, an early exit, an evaluator timeout, and a log overflow. The `spec` and `trust` validators return 0. Phase 3 is in progress as of 2026-09-01. The operator contract, the trusted transformer with anchored transformation digest chains, the layered `tx.total-timeout` invariant extension, and the isolated spike are implemented, and the `operators` and `replay` validators return 0. The spike closed as `adopt` for `time.advance/v1` on the re-registered `tx-total-timeout-resets-per-step` defect: the identity baseline is structurally rejected because it cannot advance virtual time, while the time.advance arm produces a preserved violation identity on the defect artifact, passes the clean-artifact side of the two-sided check, and reproduces five of five. The `schedule.release-order/v1` and `fault.step-outcome/v1` arms are recorded as policy-inapplicable to this invariant. Deterministic reduction to a 1-minimal spec remains before Phase 3 is complete. v0.3 is not complete. The v0.1 and v0.2 completion records below remain historical facts and are not re-judged retroactively.
